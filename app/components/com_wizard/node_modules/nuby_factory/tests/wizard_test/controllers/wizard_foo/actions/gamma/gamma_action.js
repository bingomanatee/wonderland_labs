...
on_get_input: function (rs){
	var self = this;this.models.wizard_state.get_state(function (err, state) {self.on_get_process(rs, state);}, 'wizard_foo', 'gamma');
},
...
on_post_input: function (rs,prev,next){
	if (next) {rs.go("/wizard_foo/beta");
	} else if (prev) { rs.go("/wizard_foo/gamma");
