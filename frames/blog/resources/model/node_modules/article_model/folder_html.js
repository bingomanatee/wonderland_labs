...
//	console.log('getting folder articles for %s', folder);
	...
	fa = _.filter(fa, function(a){
		return a.on_folder_homepage;
	});

	console.log('%s articles for %s: %s', fa.length, folder
	, util.inspect(_.map(fa, function(a){
			return {
				title: a.title,
				folder_homepage_weight: a.folder_homepage_weight
			}
		}))
	);
	...
	fa = _.sortBy(
		fa
	, function(a){
		return a.folder_homepage_weight || 0;
	});

	console.log('%s sorted articles for %s: %s', fa.length, folder
		, util.inspect(_.map(fa, function(a){
			return {
				title: a.title,
				folder_homepage_weight: a.folder_homepage_weight
	...
	gate.await(function(){
		...
		var html = _.reduce(articles, function(out, a){
			out.push('<h1 folder_homepage_weight="' + a.folder_homepage_weight + '">', a.title, '</h1>');
